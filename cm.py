...
class ContextManager:
    ...
    def interpolate_pose(self, img1, pose_md1, img2, pose_md2, prompt, n_prompt,
                    num_frames=10, ddim_steps=50, guide_scale=7.5):
        frames = [img1]
        self.change_mode('pose')
        ldm = self.model
        ...
        for frame in range(1,num_frames):
            pose_img = interp_poses(pose_md1, pose_md2, alpha=frame/num_frames)
            self.img2img(pose_img, mode='pose', prompt=prompt, n_prompt=n_prompt)

    @torch.no_grad()
    def img2img(self, control, prompt, n_prompt, init_img, mode=None, time_frac=0.3,
